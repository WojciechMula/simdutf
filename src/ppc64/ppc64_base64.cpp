...
simdutf_really_inline vector_u8 encoding_expand_6bit_fields(vector_u8 input) {
  ...
  #define indices4(dx, b0, b1, b2, b3)                                         \
    (dx + b0), (dx + b1), (dx + b2), (dx + b3)
  const auto expand_3_to_4 =
      vector_u8(indices4(0 * 3, 1, 0, 2, 1), indices4(1 * 3, 1, 0, 2, 1),
                indices4(2 * 3, 1, 0, 2, 1), indices4(3 * 3, 1, 0, 2, 1));
  #undef indices4
  ...
  const auto t0 =
      variable_shift_right(in, vector_u16(6, 10, 6, 10, 6, 10, 6, 10));
  ...
  const auto db = variable_shift_left(in, vector_u16(8, 4, 8, 4, 8, 4, 8, 4));
